...
export function SessionSwitcher({
  ...
  authSession;
  sessions: { number: string; theme: string; subTheme: string }[];
}) {
  ...
  async function handleSessionChange() {
    ...
    setSchoolDirectorRoles(
      allCurrentAndPastRoles
        .filter((role) => role?.roleIdentifier === "schoolDirector")
        ?.filter((role) => role?.session === selectedSession),
    );
    setDelegateRoles(
      allCurrentAndPastRoles
        .filter((role) => role?.roleIdentifier === "delegate")
        ?.filter((role) => role?.session === selectedSession),
    );
    setChairRoles(
      allCurrentAndPastRoles
        .filter((role) => role?.roleIdentifier === "chair")
        ?.filter((role) => role?.session === selectedSession),
    );
      ...
      <SidebarMenuItem>
        <DropdownMenu>
          ...
          <DropdownMenuContent
            ...
            sideOffset={4}
          >
            <DropdownMenuLabel className="text-muted-foreground text-xs">Sessions</DropdownMenuLabel>
            {sessionsData?.map((conferenceSession, index) => (
              ...
              >
                <div className="flex size-6 items-center justify-center rounded-sm border">
                  {conferenceSession.number}
                </div>
                <p className="line-clamp-1 truncate">
                  {conferenceSession.theme ||
                    `Session ${romanize(conferenceSession.numberInteger)} ${!localSelected?.isVisible ? "(Hidden)" : ""}`}
                </p>
              </DropdownMenuItem>
            ))}
            <DropdownMenuSeparator />
            ...
            >
              ...
              <div className="text-muted-foreground font-medium">Load more</div>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </SidebarMenuItem>
