...
export function AppSidebar({ sessions, authSession, ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const myCommitteeLinks = (!!delegateRoles?.length ? delegateRoles : [])
    .concat(!!chairRoles?.length ? chairRoles : [])
    .map(committeeOptions);
  ...
  const myDepartmentLinks = (!!memberRoles?.length ? memberRoles : [])
    .concat(!!managerRoles?.length ? managerRoles : [])
    .map(committeeOptions);
  ...
  const allRoles = (authSession?.user?.pastRoles || [])
    .concat(authSession?.user?.currentRoles || [])
    .filter((role) => role.session == selectedSession);
  ...
  const user = {
    ...
    email: authSession?.user.currentRoleNames[0]
      ? authSession?.user.currentRoleNames[0] == "School Director" && isChair
        ? "Chair"
        : authSession?.user.currentRoleNames[0]
      : authSession?.user.id,
  ...
  return (
    <Sidebar variant="sidebar" className="top-0 max-h-svh" collapsible={"undefined"} {...props}>
