...
export default async function Page({ params, searchParams }) {
	...
	const showButton1 = session.currentRoles.some((role) => role.name === "Manager" && role.departmentId === department.id);
	...
	const announcements = await prisma.announcement.findMany({
		...
		include: {
			DepartmentAnnouncement: { select: { id: true } },
	...
	return (
		<>
			<Drawer props={{ departmentId: department.id, sessionNumber: params.sessionNumber, departmentSlug: department.slug }} />
			<TitleBar
				title="Department Announcements"
				button1text="Create Announcement"
				button1roles={[s.management]}
				button1show={showButton1}
				button1href={`/medibook/sessions/${params.sessionNumber}/departments/${department.slug || params.departmentId}/announcements?create`}></TitleBar>
			<div className="p-5">
					...
					<div className="mt-5">
										...
										.map((announcement) => {
											return (
												<li className="list-none" key={announcement.id}>
													<Link href={`/medibook/sessions/${params.sessionNumber}/departments/${params.departmentId}/announcements/${announcement.id}`}>
						...
						<div className="mx-auto my-10 flex w-auto flex-col justify-center gap-1.5 p-3 md:flex-row">
							{!announcements.length < 10 && currentPage > 1 && (
								<Link href={`/medibook/sessions/${params.sessionNumber}/departments/${department.slug || params.departmentId}/announcements?page=1`}>
							...
							{currentPage > 1 && announcements.length > 0 && (
								<Link href={`/medibook/sessions/${params.sessionNumber}/departments/${department.slug || params.departmentId}/announcements?page=${currentPage - 1}`}>
