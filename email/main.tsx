...
export default function MainTemplate(props) {
  ...
  return (
    ...
    >
      ...
      <Html className="font-arial flex w-full flex-col items-center">
        <Section className="mx-auto mt-[40px] max-w-[600px] rounded-b-lg bg-zinc-100 px-[32px] py-[40px]">
          <Row>
        ...
        <Hr className="mx-auto my-[16px] max-w-[600px] border-t-2 bg-zinc-100" />
        {props.title && (
          <>
            <Section className="mx-auto max-w-[600px] rounded-lg bg-zinc-100 px-[40px]">
              <Heading className="font-extralight!">{props.title}</Heading>
            </Section>
            <Hr className="mx-auto my-[16px] max-w-[600px] border-t-2 bg-zinc-100" />
          </>
        )}
        <Section className="mx-auto max-w-[600px] rounded-lg bg-zinc-100 px-[40px] py-[16px] font-extralight">
          {props.children}
        </Section>
        <Hr className="mx-auto my-[16px] max-w-[600px] border-t-2 bg-zinc-100" />
        {!props.hideAd && (
          <>
            <Section className="mx-auto max-w-[600px] rounded-lg bg-zinc-100 px-[40px] py-[16px]">
              <Section>
                <Row>
                  <Text className="mt-[8px] text-[16px] font-thin leading-[24px] text-zinc-800">
                    We are a UN Simulation for teens aged 15 to 19, where students represent assigned countries. They
                    research their nation's policies to draft and debate resolutions on global issues.{" "}
                    <Link href="https://www.medimun.org">Learn more.</Link>
                  </Text>
                </Row>
              </Section>
        ...
        <Section className="mx-auto max-w-[600px] rounded-t-lg bg-zinc-100 p-[40px]">
          <Row>
            <Column colSpan={4}>
              <Img
                alt="MEDIMUN Mini Logo"
                className="rounded-full shadow-md"
                height="42"
                src={`${baseUrl}/email/logos/logo-mini.png`}
              />
              <Text className="my-[8px] text-[16px] font-light leading-[24px] text-gray-900">
                Mediterranean Model United Nations
              </Text>
              <Text className="mt-[4px] text-[10px] leading-[24px] text-gray-500">
                We are affiliated with the THIMUN Foundation.
              </Text>
            </Column>
          </Row>
        </Section>
