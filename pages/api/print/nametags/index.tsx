...
function DelegateNametag({ user, selectedSession }) {
  ...
  if (!!user.delegate?.length) {
    ...
    if (!country) {
      const selectedCommittee = selectedSession.committee.find(
        (committee) => committee.id === user.delegate[0].committeeId,
      );
    ...
    <View style={tw("w-[90mm] h-[57mm]")} key={user.id}>
      ...
      <View style={tw("absolute w-full h-full")}>
        <View style={tw("flex w-full mt-[77px] flex-col items-center justify-center")}>
          <Text style={tw(`text-[${nameColor}] text-center font-[MADEMirage] font-black text-[17.49px]`)}>
            {fullName}
          </Text>
        </View>
        <View style={tw("flex w-full flex-col items-center justify-center")}>
          <View
            style={tw(`text-[${roleColor}] text-center gap-1 flex flex-row font-[MADEMirage] font-bold text-[9.28px]`)}
          >
            <Text>{role}</Text>
            {committeeName && (
        ...
        {user?.pronouns && (
          <View style={tw("flex w-full flex-col items-center justify-center")}>
            <Text style={tw(`text-[${roleColor}] text-center font-[MADEMirage] font-bold text-[9.28px]`)}>
              {user?.pronouns}
            </Text>
          </View>
        )}
