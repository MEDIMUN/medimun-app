  ...
  const processedCommittees = selectedCommittees.map((committee) => {
    ...
    return {
      delegates: committee.delegate
        .map((delegate, index) => {
          const fullName =
            delegate.user.displayName || `${delegate.user.officialName} ${delegate.user.officialSurname}`;
          ...
          <Page key={index} size="A3" orientation="landscape" style={tw("bg-white h-full")}>
            <View style={{ display: "flex", height: "100%", width: "100%", position: "relative" }}>
              <Image
                ...
                src={`${cwd}/public/assets/pdf/placards/background.png`}
              ></Image>
            </View>
            <View style={tw("flex absolute top-0 w-full rotate-180 h-[50%] items-center justify-center")}>
              <Text
                style={{
                  ...tw("font-extrabold px-2 text-center  font-[Montserrat] translate-y-[-30px] text-[125px]"),
                  lineHeight: delegate.countryName.toUpperCase().includes("(") ? "125px" : "100px",
                }}
              >
                {delegate.countryName.toUpperCase()}
              </Text>
            </View>
            <View style={tw("flex absolute bottom-0 w-full h-[50%] items-center justify-center")}>
              <Text
                style={{
                  ...tw("font-extrabold px-2 text-center font-[Montserrat] translate-y-[-30px] text-[125px]"),
                  lineHeight: delegate.countryName.toUpperCase().includes("(") ? "125px" : "100px",
