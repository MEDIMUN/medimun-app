export function nameCase(name: string): string {
	...
	name = name.replace(/[^a-zA-Z0-9üşöçıÜŞÖÇI\-\s']/g, "");
...
export function displayNumberInSentenceAsText(inputString: string): string {
	...
	return inputString.replace(/\d/g, (digit) => numberMap[digit]);
}
