		...
		<>
			<div className="flex flex-row mx-auto max-w-[1200px]">
				...
				{status === "authenticated" &&
					session.currentRoleNames.some((role) => roles.indexOf(role) >= 0) && (
						<Button className="w-[120px] ml-2">Add New</Button>
					)}
			</div>
			{searchQuery && !isLoading && (
				<ol className="flex flex-col mx-auto max-w-[1200px] border-[1px] bg-white z-50 border-[#E2E8F0] py-[5px] mt-2 cursor-pointer rounded-md shadow-md w-full overflow-hidden">
					{data.sessions.map((session) => {
						return (
							<li>
								<Link
									key={session.id}
									className="w-full flex h-[44px] my-1 hover:bg-gray-300 bg-white  p-1"
									href={`/medibook/sessions/${session.number}`}>
										...
										<h2
											className={`${
												session.isCurrent
													...
													: "bg-black"
											} text-[24px] px-3 maxh-[40px] w-[40px] flex align-middle justify-center my-auto ml-2 mr-3 shadow-xl text-white rounded-xl`}>
					...
					{data.sessions.length == 0 && (
						<li className="w-full p-2 flex h-10 bg-white align-middle py-auto">No results found</li>
					)}
				</ol>
			)}
		</>
