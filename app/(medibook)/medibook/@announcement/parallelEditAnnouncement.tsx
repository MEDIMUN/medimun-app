 ...
 */
const MarkDownViewer = dynamic(() => import("./_components/markdown-viewer").then((mod) => mod.MarkDownViewer), {
  ssr: false,
});

export function ModalEditAnnouncement({ selectedAnnouncement, authSession }) {
  ...
  function onClose() {
    if ()
     removeSearchParams({ "edit-announcement": "" })
  ...
  const fullName =
    selectedAnnouncement?.user?.displayName ||
    `${selectedAnnouncement?.user?.officialName} ${selectedAnnouncement?.user?.officialSurname}`;
          ...
          <form id="publishAnnouncement" action={handleSubmit}>
            ...
            <section className="grid gap-x-8 gap-y-6 sm:grid-cols-2">
              <div className="space-y-1">
                ...
                <Text>
                  Select whether to display your name on the announcement or not. The management can see who shared an
                  announcement even if you hide your name.
                </Text>
              </div>
              <div className="my-auto">
                <Listbox name="privacy" defaultValue={selectedAnnouncement.privacy} disabled={isLoading}>
                  ...
                  {authorize(authSession, [s.management]) && (
                    <ListboxOption value="SECRETARIAT">Secretariat</ListboxOption>
                  )}
                  {authorize(authSession, [s.director, s.sd, s.admins]) && (
                    <ListboxOption value="BOARD">Board of Directors</ListboxOption>
                  )}
                </Listbox>
              </div>
            </section>
            ...
            <section className="grid gap-x-8 gap-y-6 sm:grid-cols-2">
              ...
              <div className="my-auto">
                <Listbox
                  name="isPinned"
                  defaultValue={selectedAnnouncement.isPinned ? "true" : "false"}
                  disabled={isLoading}
                >
            ...
            <section className="grid gap-x-8 gap-y-6 sm:grid-cols-2">
              ...
              <div className="my-auto flex flex-col gap-4 md:flex-row">
                <SlugInput
                  separator="-"
                  defaultValue={selectedAnnouncement.slug}
                  minLength={10}
                  maxLength={100}
                  name="slug"
                />
            ...
            <section className="grid gap-x-8 gap-y-6">
              <div className="space-y-1">
                ...
                <Text>
                  The content of the announcement in markdown format. Learn more about{" "}
                  <Link href={"/wiki/markdown"}>markdown</Link>
                  <br />
            ...
            {/*   <div className="w-full rounded-md bg-zinc-100 p-4">
              <ForwardRefEditor onChange={(e) => setMarkDown(e)} markdown={debouncedMarkDown} />
            </div> */}
            {debouncedMarkDown && (
