...
export const innerAnnouncementScopeList = {
  ...
  DEPARTMENTSCOPED: [
    ...
    {
      value: "DEPARTMENTSENIORDIRECTORS",
      text: "Senior Directors",
      description: "viewing Department",
      disabled: false,
    },
  ],
};
  ...
  return (
    <>
      ...
      {editAnnouncementData && (
        <ModalEditAnnouncement authSession={authSession} selectedAnnouncement={editAnnouncementData} />
      )}
    </>
  );
}
...
export const announcementWebsitecomponents = {
  ...
  a: (props) => {
    ...
    if (props.href.startsWith("http")) {
      return <a className="text-primary hover:text-primary underline" target="_blank" {...props}></a>;
    }
    return (
      <Link {...props}>
        <div className="cursor-pointer! bg-primary my-1 max-w-max rounded-md px-3 py-[2px] text-white shadow-sm duration-300 hover:shadow-md">
          {children} ↗
        </div>
      </Link>
    );
