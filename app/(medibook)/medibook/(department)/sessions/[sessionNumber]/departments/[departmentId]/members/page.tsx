...
//FIX
export default async function Page(props: { searchParams: any; params: Promise<{ sessionNumber: string; departmentId: string; page: string }> }) {
	...
	const [selectedSession, totalItems] = await prisma
		.$transaction([
			...
			prisma.member.count({
				where: {
					...whereObject,
					department: { session: { number: params.sessionNumber }, OR: [{ id: params.departmentId }, { slug: params.departmentId }] },
				},
			}),
		])
		.catch(notFound);
