...
export default async function Page(props) {
	...
	const isChairOfUser = authorizeChairDelegate(authSession.user.currentRoles, userData.currentRoles);
	const isManagerOfUser = authorizeManagerMember(authSession.user.currentRoles, userData.currentRoles);
	const isSchoolDirectorOfStudent = authorizeSchoolDirectorStudent(authSession.user.currentRoles, userData);
			...
			<div>
				...
				<div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
					<div className="-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5">
						<div className="flex">
							<Avatar showFallback className="h-24 w-24 rounded-xl ring-4 ring-white sm:h-32 sm:w-32" isBordered src={`/api/users/${userData.user.id}/avatar`} />
						</div>
						<div className="mt-6 sm:flex sm:min-w-0 sm:flex-1 sm:items-center sm:justify-end sm:space-x-6 sm:pb-1">
							<div className="mt-6 min-w-0 flex-1 sm:hidden md:block">
								<h1 className="truncate text-2xl font-bold text-gray-900">{fullName}</h1>
								{userData?.currentRoleNames[0] && <h2 className="line-clamp-2 text-large text-default-500 md:-mb-2">{userData?.currentRoleNames[0]}</h2>}
							</div>
								...
								{displayEditRolesButton && (
									<Dropdown>
										...
										<DropdownMenu>
											...
											<SearchParamsDropDropdownItem searchParams={{ "unafilliate-student": selectedUser.id }}>Unafilliate Student</SearchParamsDropDropdownItem>
											{isManagement && <SearchParamsDropDropdownItem searchParams={{ "edit-roles": selectedUser.id }}>Edit Roles</SearchParamsDropDropdownItem>}
											{isManagement && <SearchParamsDropDropdownItem searchParams={{ "assign-roles": selectedUser.id }}>Add Roles</SearchParamsDropDropdownItem>}
										</DropdownMenu>
									</Dropdown>
								)}
								<Button
