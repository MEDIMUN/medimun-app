...
const DebouncedTextarea = React.memo(({ value, onChange, placeholder, className = "" }: DebouncedTextareaProps) => {
	...
	return (
		<Textarea
			...
			className={cn(
				"!focus-visible:ring-0 focus:bg-zinc-200/80 !focus:outline-[0px] !focus:ring-[0px] min-h-[20px] right-0! border-none! outline-none! !focus-visible:outline-none shadow-none w-full resize-none overflow-hidden focus:outline-none focus:ring-0 p-0 text-sm ml-1 transition-[height] duration-100 ease-in-out",
				className
			)}
...
const DebouncedTextarea = React.memo(
	({ value, onChange, placeholder }: { value: string; onChange: (value: string) => void; placeholder: string }) => {
		...
		const debouncedOnChange = useMemo(() => debounce((value: string) => onChange(value), 500), [onChange]);
		...
		return (
			<Textarea
				value={localValue}
				onChange={handleChange}
				className={`min-h-[20px] h-auto! resize-none overflow-hidden border-none focus:outline-none focus:ring-0 p-0 text-sm w-full ml-1`}
				placeholder={placeholder}
			/>
		);
	}
	...
	({
		...
		return (
			<motion.div layout initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50 }} transition={{ duration: 0.3 }} className="mb-6">
				<div className={`p-4 bg-sidebar-accent rounded-md ${type === "preambulatory" ? "" : "border-green-300"} ${type === "preambulatory" ? "italic" : ""}`}>
