...
export default function Page(props) {
  return (
    <Suspense
      fallback={
        <main className="bg-primary flex h-screen w-full items-center justify-center overflow-x-hidden overflow-y-scroll align-middle">
...
async function CommitteeResolutions(props) {
  ...
  const isPartOfCommittee =
    isChairOfCommittee || isDelegateOfCommittee || authorize(authSession, [s.manager, s.member]);
...
async function ScResolutionPage({
  params,
  searchParams,
  authSession,
  selectedSession,
  ...
  return (
    <>
      ...
      >
        <SearchParamsButton searchParams={{ "create-alliance": selectedSession.committee[0].id }}>
          Create Alliance
        </SearchParamsButton>
      </TopBar>
      <MainWrapper>
        <Tabs defaultValue={isChairOfCommittee ? "ALL_ALLIANCES" : "MY_ALLIANCES"} className="w-full">
          <div className="showscrollbar overflow-y-hidden overflow-x-scroll">
            <TabsList className="flex h-12 w-full min-w-min max-w-max gap-2 px-3">
              ...
              <Table>
                ...
                <TableBody>
                  {MY_ALLIANCES.map((alliance) => {
                    const myCountry = allCountries.find(
                      (country) => country.countryCode === alliance.mainSubmitter.country,
                    );
                      ...
                      <TableRow key={alliance.id}>
                        <TableCell>
                          <Dropdown>
                            ...
                            <DropdownMenu>
                              <DropdownItem
                                href={`/medibook/sessions/${selectedSession.number}/committees/${selectedSession.committee[0].slug || selectedSession.committee[0].id}/resolutions/alliances/${alliance.id}`}
                              >
                                View
                              </DropdownItem>
                              <SearchParamsDropDropdownItem searchParams={{ "invite-alliance-member": alliance.id }}>
                                Invite Member
                              </SearchParamsDropDropdownItem>
                              <SearchParamsDropDropdownItem searchParams={{ "delete-alliance": alliance.id }}>
                                Delete
                              </SearchParamsDropDropdownItem>
                            </DropdownMenu>
                          </Dropdown>
          ...
          <TabsContent value="ALLIANCE_INVITATIONS">
              ...
              <ul className="mt-6">
                {ALLIANCE_INVITATIONS.map((invitation) => {
                  ...
                  const selectedCountry = allCountries?.find(
                    (country) => country?.countryCode === invitation?.alliance.mainSubmitter.country,
                  )?.countryNameEn;
                  const fullName =
                    invitation.alliance.mainSubmitter?.user?.displayName ||
                    `${invitation.alliance.mainSubmitter?.user?.officialName} ${invitation.alliance.mainSubmitter?.user?.officialSurname}`;
                  return (
                    <div
                      key={invitation.id}
                      className="flex flex-col items-center justify-between gap-4 border-y border-gray-200 p-4 md:flex-row"
                    >
                      <div>
                        <p className="font-semibold">
                          Alliance A{invitation.alliance.number.toString().padStart(5, "0")}
                        </p>
            ...
            <Paginator
              ...
              totalItems={ALLIANCE_INVITATIONS_LENGTH}
              itemsOnPage={ALLIANCE_INVITATIONS.length}
            />
          </TabsContent>
          <TabsContent value="JOINED_ALLIANCES">
                        ...
                        <TableCell>
                          <Dropdown>
                            ...
                            <DropdownMenu>
                              ...
                              <SearchParamsDropDropdownItem searchParams={{ "leave-alliance": alliance.id }}>
                                Leave
                              </SearchParamsDropDropdownItem>
                            </DropdownMenu>
                          </Dropdown>
            ...
            <Paginator
              itemsPerPage={perPage}
              totalItems={JOINED_ALLIANCES_LENGTH}
              itemsOnPage={JOINED_ALLIANCES.length}
            />
          </TabsContent>
        </Tabs>
      </MainWrapper>
...
async function GaReslutionPage({
  params,
  searchParams,
  authSession,
  selectedSession,
  ...
  const [
    myResolutions,
    numberOfMyResolutions,
    coSubmitterInvitations,
    coSubmittingResolutions,
    coSubmittingResolutionsLength,
  ...
  if (isChairOfCommittee || isManagement) {
    [
      ...
      DRAFTLENGTH,
      SENT_TO_CHAIRS,
      SENT_TO_CHAIRSLENGTH,
      SENT_TO_APPROVAL_PANEL,
      SENT_TO_APPROVAL_PANELLENGTH,
      SENT_BACK_TO_COMMITTEE,
      SENT_BACK_TO_COMMITTEELENGTH,
    ] = await prisma.$transaction([
      prisma.resolution.findMany({
        where: {
          committeeId: selectedSession.committee[0].id,
  ...
  if (isChairOfCommittee || isManagement) {
    [DRAFT, DRAFTLENGTH, SENT_TO_CHAIRS, SENT_TO_CHAIRSLENGTH, SENT_TO_APPROVAL_PANEL, SENT_TO_APPROVAL_PANELLENGTH] =
      await prisma.$transaction([
        prisma.resolution.findMany({
          where: {
            committeeId: selectedSession.committee[0].id,
      ...
      <MainWrapper>
        <Tabs defaultValue={isChairOfCommittee ? "DRAFT" : "main_resolutions"} className="w-full">
            ...
            <TabsList className="flex h-12 w-full min-w-min max-w-max gap-2 px-3">
              ...
              {(isChairOfCommittee || isManagement) && (
                <TabsTrigger value="DRAFT">
                  Draft <Badge className="rounded-full! ml-1"> Stage 1 </Badge>
                </TabsTrigger>
              )}
              {isDelegateOfCommittee && <TabsTrigger value="invitations">Co-Submitter Invitations</TabsTrigger>}
              {(isChairOfCommittee || isManagement) && (
                <TabsTrigger value="SENT_TO_CHAIRS">
                  Submitted to Chairs <Badge className="rounded-full! ml-1"> Stage 2 </Badge>
                </TabsTrigger>
              )}
              {(isChairOfCommittee || isManagement) && (
                <TabsTrigger value="SENT_TO_APPROVAL_PANEL">
                  Sent to Approval Panel
                  <Badge className="rounded-full! ml-1"> Stage 3 </Badge>
                </TabsTrigger>
              )}
              {(isChairOfCommittee || isManagement) && (
                <TabsTrigger value="SENT_BACK_TO_COMMITTEE">To Be Debated</TabsTrigger>
              )}
              <TabsTrigger className="text-red-500!" value="FAILED">
                Failed Resolutions
              </TabsTrigger>
                    ...
                    <TableRow key={resolution.id}>
                      <TableCell>
                        <Dropdown>
                          ...
                          <DropdownMenu>
                            <DropdownItem
                              href={`/medibook/sessions/${selectedSession.number}/committees/${selectedSession.committee[0].slug || selectedSession.committee[0].id}/resolutions/${resolution.id}`}
                            >
                              View
                            </DropdownItem>
                            {((resolution.status === "DRAFT" &&
                              resolution.mainSubmitter.userId === authSession.user.id) ||
                              isManagement) && (
                              <SearchParamsDropDropdownItem
                                searchParams={{ "delete-committee-resolution": resolution.id }}
                              >
                                Delete
                              </SearchParamsDropDropdownItem>
                            )}
          ...
          <TabsContent value="co_resolutions">
            ...
            <Paginator
              itemsPerPage={perPage}
              totalItems={coSubmittingResolutionsLength}
              itemsOnPage={coSubmittingResolutions.length}
            />
          </TabsContent>
          <TabsContent value="invitations">
            <ul className="mt-6">
              {coSubmitterInvitations.map((invitation) => {
                const allCountries = countries;
                const selectedCountry = allCountries?.find(
                  (country) => country?.countryCode === invitation?.resolution.mainSubmitter.country,
                )?.countryNameEn;
                const fullName =
                  invitation.resolution.mainSubmitter?.user?.displayName ||
                  `${invitation.resolution.mainSubmitter?.user?.officialName} ${invitation.resolution.mainSubmitter?.user?.officialSurname}`;
                return (
                  <div
                    key={invitation.id}
                    className="flex flex-col items-center justify-between gap-4 border-y border-gray-200 p-4 md:flex-row"
                      ...
                      <TableCell>
                        <Dropdown>
                          ...
                          <DropdownMenu>
                            ...
                            {(isChairOfCommittee || isManagement) && (
                              <ForceSendToChairs resolutionId={resolution?.id} />
                            )}
                        ...
                        <Dropdown>
                          ...
                          <DropdownMenu>
                            ...
                            {((resolution.status === "DRAFT" &&
                              ...
                              isManagement ||
                              (isChairOfCommittee &&
                                ["DRAFT", "SENT_BACK_TO_COMMITTEE", "SENT_TO_CHAIRS", "IN_DEBATE", "VOTING"].includes(
                                  resolution.status,
                                ))) && (
                              <SearchParamsDropDropdownItem
                                searchParams={{ "delete-committee-resolution": resolution.id }}
                            ...
                            {(isChairOfCommittee || isManagement) && resolution.status === "SENT_TO_CHAIRS" && (
                              <SearchParamsDropDropdownItem
                                searchParams={{ "send-resolution-to-approval": resolution.id }}
                              >
                                Send for Approval
                              </SearchParamsDropDropdownItem>
                            )}
                            {(isChairOfCommittee || isManagement) && resolution.status === "SENT_TO_CHAIRS" && (
                              <MakeDraftAgainButton resolutionId={resolution.id} />
                            )}
                          </DropdownMenu>
                        </Dropdown>
                      </TableCell>
          ...
          <TabsContent value="SENT_TO_APPROVAL_PANEL">
            ...
            <Paginator
              itemsPerPage={perPage}
              totalItems={SENT_TO_APPROVAL_PANELLENGTH}
              itemsOnPage={SENT_TO_APPROVAL_PANEL.length}
            />
          </TabsContent>
          {(isChairOfCommittee || isManagement) && (
            <TabsContent value="SENT_BACK_TO_COMMITTEE">
              ...
              <Paginator
                itemsPerPage={perPage}
                totalItems={SENT_BACK_TO_COMMITTEELENGTH}
                itemsOnPage={SENT_BACK_TO_COMMITTEE.length}
              />
            </TabsContent>
          )}
