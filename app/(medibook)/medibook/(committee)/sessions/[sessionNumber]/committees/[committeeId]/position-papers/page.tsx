import {
  ...
  TopBar,
  UserTooltip,
} from "@/app/(medibook)/medibook/client-components";
...
export function arePositionPaperSubmissionsOpen(selectedCommittee: Committee) {
  ...
  if (!selectedCommittee.positionPaperSubmissionAutoOpenTime || !selectedCommittee.positionPaperSubmissionAutoCloseTime)
    return false;
  return (
    selectedCommittee.positionPaperSubmissionAutoOpenTime < now &&
    selectedCommittee.positionPaperSubmissionAutoCloseTime > now
  );
}
...
export default async function Page(props) {
  ...
  const canSubmit =
    (!!lastPositionPaper && ["REVISION"].includes(lastPositionPaper?.status)) ||
    otherPositionPapersOfDelegate.length == 0;
  ...
  return (
    <>
      <TopBar
        config={isManagementOrChair ? <SubmissionOptions selectedCommittee={selectedCommittee} /> : null}
        hideBackdrop
      ...
      <MainWrapper>
        <div className="rounded-md bg-zinc-950/5 p-4">
          ...
          {selectedCommittee?.positionPaperSubmissionAutoCloseTime &&
            selectedCommittee.isPositionPaperSubmissionAutoOpen && (
              <Text className="text-xs!">
                You can submit a position paper{" "}
                {selectedCommittee?.positionPaperSubmissionAutoOpenTime && ` starting from `}
                {selectedCommittee?.positionPaperSubmissionAutoOpenTime
                ...
                <TableBody>
                  {positionPapers.map((paper) => {
                    ...
                    const selectedCountry = allCountries.find(
                      (country) => country.countryCode === user?.delegate[0]?.country,
                    );
                    return (
                        ...
                        <TableCell>
                          <Dropdown>
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownItem href={`/medibook/users/${user.username || user.id}`}>
                                View User
                              </DropdownItem>
                              <DropdownItem href={`/medibook/messenger/@${user.username || user.id}?new=true`}>
                                Message User
                              </DropdownItem>
                              <SearchParamsDropDropdownItem searchParams={{ "return-position-paper": paper.id }}>
                                Return
                              </SearchParamsDropDropdownItem>
                              <SearchParamsDropDropdownItem searchParams={{ "delete-position-paper": paper.id }}>
                                Delete
                              </SearchParamsDropDropdownItem>
                            </DropdownMenu>
                          </Dropdown>
        ...
        {isDelegateOfCommittee && !isManagement && !isChairOfCommittee && (
          <div className="rounded-md bg-zinc-100 p-4">
            ...
            {!!positionPapersOfDelegate.length ? (
              ...
              <div className="m-4 mx-auto max-w-max rounded-lg p-4 text-center">
                <svg
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                ...
                <p className="mt-1 text-sm text-gray-500">
                  No position papers have been submitted by you for this committee.
                </p>
                <div className="mt-6">
                  <SearchParamsButton disabled={!areSubmissionsOpen} searchParams={{ "submit-position-paper": true }}>
                    Submit New
                          ...
                          <TableCell>
                            <UserTooltip userId={user.id}>
                              <div className="flex gap-2">
                                <Avatar
                                  className="my-auto h-5 w-5 rounded-sm"
                                  showFallback
                                  radius="md"
