		...
		<main>
			<TopBar title="All Sessions" defaultSort="nameasc" searchText="Search sessions..." hideSearchBar={false}>
				{authorize(authSession, [s.admins, s.sd]) && (
					<div className="w-full gap-4 md:w-auto">
						<Button className="w-full">New session</Button>
					</div>
				)}
			</TopBar>
			{currentPage !== 1 && <Divider className="mt-10" />}
			<ul className="">
					...
					return (
						<>
							<li
								className={cn(
									"bg-cover",
									session?.isCurrent && "mb-6 mt-10 overflow-hidden rounded-lg bg-center text-zinc-800 shadow-md duration-300"
								)}
								style={
									session.isCurrent
										? session.coverImage
			...
			<Paginator itemsPerPage={sessionsPerPage} totalItems={numberOfSessions} />
		</main>
	);
}
