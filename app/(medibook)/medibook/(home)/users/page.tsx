		...
		<>
			...
			<SelectedContextProvider defaultUserData={editUsers}>
				...
				<div>
							...
							{users.map((user) => (
								<TableRow key={user.id}>
									<TableCell>
										<UserSelector
											className="mt-2"
											isDisabled={highestRoleRank >= user.highestRoleRank}
											uid={user.id}
					...
					<Table className="showscrollbar -md:block absolute -hidden -translate-y-[calc(100%+11px)] border-r-1 bg-white">
						<TableHead>
							<TableRow>
			...
			<Paginator totalItems={numberOfUsers} itemsPerPage={usersPerPage} />
		</>
	);
}
