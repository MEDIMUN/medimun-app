					...
					<Card key={announcement.id} className={`h-[200px] ${announcement.isPinned && "shadow-md shadow-slate-400"}`}>
						...
						<CardFooter className="flex flex-row gap-2 bg-gray-100">
							{announcement.privacy !== "ANONYMOUS" && (
								<User
									name={announcement.user.displayName || announcement.user.officialName + " " + announcement.user.officialSurname}
									avatarProps={{ src: `/api/users/${announcement.user.id}/avatar`, size: "sm", isBordered: true, name: announcement.user.officialName[0] + announcement.user.officialSurname[0], showFallback: true, color: "primary", className: "min-w-max" }}
								/>
							)}
