...
export default function SearchBar({ props }) {
	...
	useEffect(() => {
		setIsOpen(
			searchParams.get("create") == "" &&
				...
				(authorize(session, [s.management]) || session.currentRoles.some((role) => role.name === "Manager" && role.departmentId == props.departmentId))
		);
	...
	async function createAnnouncementWrapper(formData) {
		...
		if (res?.ok) {
			router.push(`/medibook/sessions/${props.sessionNumber}/departments/${props.departmentSlug || props.departmentId}/announcements`);
			router.refresh();
		}
	}
	...
	return (
		<Sheet
			onOpenChange={() => {
				router.push(`/medibook/sessions/${props.sessionNumber}/departments/${props.departmentId}/announcements`);
			}}
			open={isOpen}>
			<SheetContent className="overflow-y-auto" position="right" size="content">
				<SheetHeader>
					<SheetTitle>Create an Announcement</SheetTitle>
					<SheetDescription>Announcements published from here will only be visible in this department.</SheetDescription>
				</SheetHeader>
				<form action={createAnnouncementWrapper} id="main" name="main" className="flex flex-col gap-4 pb-2 pt-4">
					<Input required name="departmentId" id="committeeId" type="hidden" value={props.departmentId} />
