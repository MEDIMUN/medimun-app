...
export async function createAnnouncement ( formData ) {
   ...
   if ( !departmentId || !title || !description || !markdown ) return { ok: false, error: "Invalid input 1", title: "Invalid input", variant: "destructive" };
   if ( typeof departmentId !== "string" || typeof title !== "string" || typeof description !== "string" || typeof markdown !== "string" ) return { ok: false, error: "Invalid input 2", title: "Invalid input", variant: "destructive" };
   ...
   if ( !( authorize( session, [ s.management ] ) || session.currentRoles.some( ( role ) => role.name === "Manager" && role.departmentId === departmentId ) ) ) {
      return { ok: false, error: "You are not authorized to create announcements." };
   }
