...
export async function createAnnouncement ( formData ) {
   ...
   const currentChairRoles = currentRoles.filter( role => role.committeeId === formData.get( "committeeId" ) && role.name === "chair" );
   ...
   if ( !newAnnouncement.committeeId || !newAnnouncement.title || !newAnnouncement.markdown || !newAnnouncement.privacy )
   ...
   if ( newAnnouncement.privacy === "ANONYMOUS" && !authorize( session, [ s.management ] ) )
      return { ok: false, error: "Unauthorized", title: "Unauthorized", description: "You are not authorized to perform this action", variant: "destructive" };
   try {
      await prisma.committeeAnnouncement.create( {
