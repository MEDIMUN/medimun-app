...
export default function SearchBar({ props }) {
	...
	async function createAnnouncementWrapper(formData) {
		...
		formData.append("committeeId", props.committeeId);
		...
		<Sheet
			...
			<SheetContent className="overflow-y-auto" position="right" size="content">
				...
				<form action={createAnnouncementWrapper} id="main" name="main" className="flex flex-col gap-4 pb-2 pt-4">
					<Input label="Title" labelPlacement="outside" placeholder=" " minLength={3} isRequired maxLength={128} name="title"></Input>
					<Input label="Description" labelPlacement="outside" placeholder=" " minLength={10} maxLength={200} name="description" />
					<RadioGroup defaultValue="NORMAL" size="sm" name="privacy" label="Publisher Privacy Options">
					...
					<Button isLoading={isLoading} isDisabled={isLoading} type="submit" className="ml-auto">
						Publish
					</Button>
				</form>
			</SheetContent>
