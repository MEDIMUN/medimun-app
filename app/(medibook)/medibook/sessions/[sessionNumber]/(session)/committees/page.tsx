...
export default async function Component({ params, searchParams }) {
	...
	const currentCommitteeIds = [...authSession.currentRoles, ...authSession.pastRoles]
	...
	committees = committees.sort((a: any, b: any) => {
	...
	const totalItems = await prisma.committee.count({ where: { session: { number: params.sessionNumber } } }).catch();
	...
	return (
		<>
			<TopBar
				...
				title="Committees">
				{isManagement && <SearchParamsButton searchParams={{ createcommittee: "" }}>Create New</SearchParamsButton>}
			</TopBar>
			<ul className="mt-10">
							...
							<li key={committee.id}>
								...
								<div className="flex items-center justify-between">
										...
										<div className="w-32 shrink-0">
											<Link href={`/medibook/sessions/${params.sessionNumber}/committees/${committee.slug || committee.id}`} aria-hidden="true">
												<img
													...
													src={committee?.cover ? `/api/committees/${committee.id}/cover` : `/gradients/${((index + 1) % 6) + 1}.jpg`}
									...
									<div className="flex items-center gap-4">
										<CommitteeDropdown committee={committee} isManagement={isManagement} params={params} />
									</div>
								</div>
							</li>
						</>
