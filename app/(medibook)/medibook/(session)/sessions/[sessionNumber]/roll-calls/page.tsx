...
export async function RollCallsPage(props) {
  ...
  const [days, rollCalls] = await prisma
    .$transaction([
      ...
      prisma.rollCall.findMany({
        where: { day: { id: searchParams?.["selected-day"], session: { number: params.sessionNumber } } },
        orderBy: { index: "asc" },
        include: { day: true },
      }),
    ])
  ...
  if (!searchParams?.["selected-day"] && currentDayId)
    redirect(`/medibook/sessions/${params.sessionNumber}/roll-calls?selected-day=${currentDayId}`);
  ...
  return (
    <>
      ...
      >
        <DateSelector
          className="ml-auto md:max-w-[300px]"
                  ...
                  <TableCell>
                    <Dropdown>
                      ...
                      <DropdownMenu>
                        ...
                        <SearchParamsDropDropdownItem searchParams={{ "edit-roll-call": rc.id }}>
                          Edit
                        </SearchParamsDropDropdownItem>
                        <SearchParamsDropDropdownItem searchParams={{ "delete-roll-call": rc.id }}>
                          Delete
                        </SearchParamsDropDropdownItem>
                      </DropdownMenu>
                    </Dropdown>
