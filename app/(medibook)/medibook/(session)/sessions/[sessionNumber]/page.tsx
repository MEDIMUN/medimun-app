...
export default async function Page(props) {
  ...
  let selectedSession = prisma.session.findUnique({
    ...
    include: {
      secretaryGeneral: {
        select: {
          user: { select: { officialName: true, officialSurname: true, displayName: true, username: true, id: true } },
        },
      },
  ...
  [numberOfNationalities, numberOfSchools, selectedSession] = await Promise.all([
    ...
    <>
      ...
      <div className="flex h-[200px] w-full overflow-hidden bg-[url(/assets/medibook-session-welcome.webp)] bg-cover bg-right shadow-md md:h-[328px]">
        <div className="mt-auto p-5">
          <p className="text-primary mb-1 font-[canela] text-2xl md:text-4xl">{selectedSession.theme}</p>
          <p className="font-[canela] text-xl text-zinc-700 md:text-2xl dark:text-zinc-300">
            {selectedSession.subTheme}
          </p>
        </div>
      </div>
      {selectedSession.isVisible && (
        <dl className="grid grid-cols-2 gap-0.5 divide-gray-800 overflow-hidden text-center shadow-md sm:grid-cols-2 lg:grid-cols-4">
          {statsArray.map((stat, index) => (
            <div key={index + "statcard"} className="flex flex-col bg-gray-300/5 p-8">
              <dt className="text-sm font-semibold leading-6 text-gray-600 dark:text-white">{stat.title}</dt>
              <dd className="order-first text-3xl font-semibold tracking-tight text-gray-900 dark:text-zinc-300">
                {stat.data}
              </dd>
            </div>
          ))}
