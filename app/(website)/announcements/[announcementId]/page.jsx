...
export default async function Page({ params }) {
	...
	const fullName = announcement.isAnonymous ? "Anonymous" : user?.fullName;
	const publishedDate = announcement?.time.toLocaleString("en-uk", { month: "long", day: "numeric", year: "numeric" });
	return (
		<>
			<div className="z-[40] h-auto min-h-screen w-full bg-gradient-to-r from-gray-100 via-gray-200 to-gray-300 font-[Montserrat] text-xl font-[700] text-black">
				<div className="mx-auto flex w-full max-w-[1200px] flex-col gap-10 pt-[96px] md:p-4 md:pt-[96px]">
					<div>
						<Link href="/announcements">
							<Button className="mb-4 ml-4 h-[20px]  rounded-[50px] bg-white text-[10px] text-black duration-500 hover:bg-primary hover:text-white">
								← ALL ANNOUNCEMENTS
							</Button>
						</Link>
...
const ResponsiveImage = (props) => {
	if (props.src.includes("http")) {
		return (
			<img
				className="px-auto w-full rounded-sm bg-gray-100 shadow-lg duration-300 hover:shadow-xl md:w-[70%]"
				alt="custom image"
				width={100}
				height={100}
				{...props}
...
const a = (props) => {
	...
	return (
		<Link
			truncated={truncated}
			className="max-w-[300px] truncate tracking-tight text-blue-700 duration-300 after:content-['_↗'] hover:rounded-3xl hover:bg-primary hover:p-2 hover:px-4 hover:text-white hover:shadow-lg hover:after:content-['_(Internal_Navigation:'attr(truncated)')_↗']"
			{...props}
		/>
	);
};
