...
export default function Page() {
	...
	const SummaryButton = () => {
		return (
			<div className={`ml-4 mt-2 flex w-max items-center space-x-2 bg-gray-100 p-2 duration-1000 ${isSummary && "rounded-sm px-2"}`}>
				<Switch className="bg-[var(--medired)]" checked={isSummary} onCheckedChange={(e) => setIsSummary(e)} id="summaryMode" />
				<Label className="text-lg text-black" htmlFor="summaryMode">
					Summary mode
				</Label>
			</div>
		);
	...
	const highlightStyle = `duration-1000 bg-gray-200 py-1 hover:bg-black hover:cursor-none w-max ${isSummary && "px-1 rounded-md -ml-[8px]"}`;
	const highlightStyle2 = `duration-1000 hover:text-[var(--medired)] bg-gray-200 py-1 hover:bg-black hover:cursor-none ${isSummary && "px-1 rounded-md -ml-[8px]"}`;
		...
		<div className="-bg-[url(/assets/delegates-indoors-2.jpg)] px h-auto min-h-[100vh] w-full bg-medired  bg-gradient-to-r from-gray-700 via-gray-900 to-black bg-cover font-[Montserrat] text-xl font-[700] text-white">
			<div className="mx-auto flex h-auto min-h-[100vh] w-full max-w-[1200px] flex-col gap-10 p-4 pt-[96px]">
				<section>
					...
					<SummaryButton />
					<p className="h-auto bg-opacity-50 p-4 text-sm leading-7 md:text-lg md:leading-9">
						<span className={`py-1 text-[var(--medired)] duration-1000 ${isSummary ? "max-w-full overflow-auto" : "bg-gray-200"}`}>
							...
							<span className={`${isSummary && normalStyle}`}> Model United Nations conference in Cyprus. Administered under the </span>
				...
				<section>
					...
					<SummaryButton />
					<p className="h-auto bg-opacity-50 p-4 text-sm leading-7 md:text-lg md:leading-9">
						<span className={`py-1 text-gray-800 duration-1000 ${isSummary ? "max-w-full overflow-auto" : "bg-gray-200"}`}>
