...
type selectedResolution = Prisma.AllianceGetPayload<{
	include: {
		committee: {
			include: {
				session: true;
...
export default async function ResolutionPageSC(props) {
	const authSession = await auth();
	if (!authSession) return notFound();
	const isManagement = authorize(authSession, [s.management]);
	const params = await props.params;

	const selectedAlliance = await prisma.resolution.findUnique({
